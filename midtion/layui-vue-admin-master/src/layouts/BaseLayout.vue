    ...
    >
      ...
      >
        <lay-logo v-if="appStore.logo">
          <p id="wordbox" @click="collapse">
  <template v-if="state.isOdd">
    <div id="img1111"></div>
  </template>
  <template v-else>
    <!-- 显示文字的代码 -->
              ...
              <lay-dropdown updateAtScroll placement="bottom">
                ...
                <template #content>
                  <lay-dropdown-menu>
                    ...
                    <lay-dropdown-menu-item @click="toUserInfoo">
                      <template #default>修改密码</template>
                    </lay-dropdown-menu-item>
                    <lay-dropdown-menu-item @click="clearuser">
                      <template #default>退出账户</template>
                    </lay-dropdown-menu-item>
...
export default {
  ...
  isOdd: false,
    ...
    const takelString = localStorage.getItem('taken');
    // 检查takelString是否存在，因为如果没有存储任何值，getItem会返回nu
    if (takelString) {
      console.log("死妈玩意")
    }else if(datag.takel) {
       console.log("死妈")
...
#wordbox{
  ...
  text-overflow: ellipsis; /* 溢出部分以省略号表示 */
}
