...
<script>
import ReplacementComponent from './ThrePage.vue';
...
const name = datag.school.value.schoolName || (JSON.parse(localStorage.getItem('schooll')) || {}).schoolName || '无';
    const openConfirm = async(row)=>{layer.confirm(row.auditRemark)}
    const dataSource = ref([])
    const lxxdata1=async()=>{
      ...
      try{
        ...
        datag.stuR.value = data.data
        dataSource.value =  datag.stuR.value
    ...
    const columns = ref([
      ...
      { title:"专业代码", width: "50px", key:"spmcode", sort: "desc" },
      { title:"专业名称", width: "50px", key:"spmname",},
      { title:"原计划数", width: "40px", key:"orginal_plan_number", },
