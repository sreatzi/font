...
const name = datag.school.value.schoolName || (JSON.parse(localStorage.getItem('schooll')) || {}).schoolName || '无';
    datag.Ae6a5bde689bde698afe4bda2e58aa2e4bb9be8bf9ee4baa5e4bb94e6af92=true
    const lxxcity = async()=>{
      
    const lxx1 = supapi.supapi+"/api/SecondaryEnrollment/getregion?key=local"
    const headers = {
            method:"GET",
            headers:{
  ...
  const lxxapi = async()=>{
    ...
    const lxx = supapi.supapi+"/api/SecondaryEnrollment/getprofessionalplans?transcribe=3&exception=1"
      const headers = {
        method:'GET',
        headers:{
          'Authorization':'Bearer '+(JSON.parse(localStorage.getItem('taken'))?.accessToken || datag.takel.accessToken)
...
onMounted(()=>{lxxapi()})
    ...
    const columns8 = [
      ...
      {title:"计划处", width:"200px", key:"number",customSlot:"operator"},
    ]
    ...
    watch(selectedKey3, (newValue) => {
     ...
     .then(data=>{
        console.log(data)
        datag.stuRcity.value = data.data
        if(datag.stuRcity.value==null){
        console.log("有值为空")
        ...
        dataSource8.value.forEach((item,index)=>{
        item.number = datag.stuRcity.value[index].number
      })
