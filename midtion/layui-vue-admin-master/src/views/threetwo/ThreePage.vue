...
const name = datag.school.value.schoolName || (JSON.parse(localStorage.getItem('schooll')) || {}).schoolName || '无';
    const addlxxstu=async()=>{
      const lxx = supapi.supapi + "/api/SecondaryEnrollment/getRelationmajor?Transcribe=2"
        const headers = {
            method:"GET",
            headers:{
