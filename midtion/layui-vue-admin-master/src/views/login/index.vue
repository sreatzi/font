...
const tkk = async()=>{
    ...
    const loginSubmit = async () => {
        ...
        const lxxlc = supapi.supapi + "/api/OAuth/secondarylogin";
        let lxxdata = {
          Account: username,
          Password: password
        };
              ...
              layer.msg(data.message, { icon: 1 }, async () => {
                ...
                if(data.data.first ==1){
                  ...
                  router.push('/main')
                    ...
                    if(stats.value == 1){
                      console.log("转")
                      localStorage.setItem('password', JSON.stringify(datag.password));
                      localStorage.setItem('schooll',JSON.stringify(datag.school.value))
                      localStorage.setItem('taken', JSON.stringify(datag.takel));
                    }else if(stats.value == 0){
                      console.log("转1")
                      localStorage.clear()
                    }
                    console.log(JSON.parse(localStorage.getItem('taken')))
