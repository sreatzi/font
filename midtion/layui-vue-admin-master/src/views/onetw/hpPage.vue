...
const name = datag.school.value.schoolName || (JSON.parse(localStorage.getItem('schooll')) || {}).schoolName || '无';
    ...
    const ppppid=[]
        ...
        datag.stuootcity.value.forEach(item=>{
    ...
    const shenbao = async(row) =>{
      ...
      const fechdata = async()=>{
        ...
        try{
          ...
          datag.stuootcity.value =[]
        dataSource.value.forEach(item =>{
          if(item.state==0||item.state==3)
          {
            datag.stuootcity.value.push(item)
          }
        })
        }catch(error){
      ...
      onMounted(()=>{fechdata()})
      watch(datag.stuone, () => {
        if(dataSource.value==datag.stuone.value)
        {
            console.log("一样的")
        }else{
          fechdata();
      ...
      const columns = ref([
        ...
        { title:"专业代码", width: "50", key:"spmcode", fixed: "left", sort: "desc" },
        { title:"专业名称", width: "50", key:"spmname", sort: "desc" },
